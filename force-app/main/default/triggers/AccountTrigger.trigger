...
// Trigger.new is literally List<Account> that entered the trigger 
  ...
  if (Trigger.isAfter && Trigger.isInsert) {
    ...
    // We are about to insert contacts for each Account entered the trigger, so we need a List<Contact>
    // to store it and eventually insert this using DML outside the Loop 
    ...
    for(Account each : Trigger.new) {
      if(each.AnnualRevenue>1000000){
        ...
        // fill up the field value and add it ot the list 
        contactList.add(new Contact(Account Id= each.Id, LastName ='Snow'));
